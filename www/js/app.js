...
jQuery(function($){
  ...
  $(document).on('click', ".mobile-home-btn", function(e){
    $( document ).find('.active-row').addClass('hide').removeClass('active-row');
    $(".home-row").removeClass('hide').addClass('active-row');
  });
  //nav link navigation
     
  $(document).on('click', "#load_content a", function(e){
    if($(this).attr("nav-link")){
      var page    = $(this).attr("nav-link"); 
      console.log(page);
      
      if(page){
        loadingShow();
        $("div .active-row").addClass('hide').removeClass('active-row');
        $("."+page).addClass('active-row').removeClass('hide');

        var variables     = page.slice(page.indexOf('?') + 1).split('&');
        var num_variables = variables.length - 1;
        var url           = page.substr(0, page.indexOf('?'));
        var actions;

        if(url == "")
          actions = page.replace("-", "_");
        else
          actions = url.replace("-", "_");
  ...
  $("form").submit(function(){
    ...
    //validate
    $(form+' .required_field').each(function(i){
      ...
      if(i == $(form+' .required_field').length - 1){
  ...
  //PIC UPLOAD
  $(document).on('click', "#sudo-pic-load a", function(e){
    $('#pic-load').click();
    return false;
  });

  //GET DIRECTIONS//PENDING
  $(document).on('click', "#get_directions a", function(e){
    if(navigator.geolocation) {
      browserSupportFlag = true;
      navigator.geolocation.getCurrentPosition(function(position) {
        var position_from   = {lat:position.coords.latitude, lng:position.coords.longitude};
        var position_to     = {lat: emergency.latitude, lng: emergency.longitude};

        var mapOptions      = {center: position_from, zoom: 7} 
        ...
        var request = {
          destination: position_to,
          origin: position_from,
        ...
        var content = "We are unable to determine your current location and so directional services are not available.";
        var title = "Direction Services";
